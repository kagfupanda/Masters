...
/*
 * File: sumwith.4.cpp (implemented WorkerThread and total sum)
 ...
 * Build: 
 *       g++ sumwith.4.cpp -lpthread -o sumwith.cpp
 * or add -DDEBUG to enable debug prints
 *       g++ -DDEBUG sumwith.4.cpp -lpthread -o sumwith.cpp
 * To run:
 *   ./sumwith 20 4 
 ...
 *
 * NOTE:
 * Parallel Sum has benefits only when array is HUGE
 * and number of workers proportion to CORES/CPUS and each
 * worker does LARGE number of elements (NUM_ELEMENTS_PER_WORKER high).
 *   ./sumwith 4000000 8
 * To parallel sum array of 4 MILLION floats with 8 workers
 */
...
const int NUM_ELEMENTS_PER_WORKER = 256*1024; // Number of array elements to SUM by Worker
...
int main(int argc, char **argv)
{
	...
	std::cout << "Serial sum: " << serial_sum
		 << " took " << std::scientific << time_taken.count() << " seconds"
	     << std::endl;
	...
	std::cout << "Parallel sum: " << total_sum
	     << " took " << std::scientific << ptime_taken.count() << " seconds."
	     << std::endl;
